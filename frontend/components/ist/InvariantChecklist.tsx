...
import { getInvariants } from "@/lib/api/ist";
import type { InvariantResult, InvariantsResponse } from "@/types/ist";
import {
  AlertCircle,
  Loader2,
  CheckCircle,
...
function InvariantItem({ invariant }: { invariant: InvariantResult }) {
  ...
  return (
    <div
      className={cn(
        ...
        isPassed ? "hover:bg-emerald-500/10" : "hover:bg-red-500/10 bg-red-500/5"
      )}
    >
      {/* Status icon */}
      {isPassed ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          <span className={cn(
            "text-xs font-medium",
            isPassed ? "text-text-primary" : "text-red-400"
          )}>
            {invariant.name}
          </span>
        </div>
...
export default function InvariantChecklist({ screenId }: InvariantChecklistProps) {
  const [data, setData] = useState<InvariantsResponse | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getInvariants(screenId);
      setData(result);
    } catch (err) {
    ...
    return (
      <div className="bg-[rgba(10,15,26,0.6)] backdrop-blur-[12px] border border-border rounded-xl px-6 py-12 text-center">
        ...
        <p className="text-xs text-text-secondary mt-1">
          Invariant checks will appear here once the scanning phase completes.
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-5 gap-3">
        <div>
          ...
          <p className="text-xs text-text-secondary mt-0.5">
            Quality gates for screening integrity
          </p>
        </div>
