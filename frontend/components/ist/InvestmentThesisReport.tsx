...
import { getReport } from "@/lib/api/ist";
import type { ReportResponse } from "@/types/ist";
import {
  AlertCircle,
  Loader2,
  FileText,
...
/**
 * Parses a markdown string into structured blocks for rendering.
 * Handles: headings (h1-h3), blockquotes, tables, ordered/unordered lists,
...
function parseMarkdown(raw: string): { blocks: MdBlock[]; toc: TocEntry[] } {
  ...
  while (i < lines.length) {
    ...
    // Table: detect header row with pipe separators followed by separator row
    if (
      line.includes("|") &&
      i + 1 < lines.length &&
      /^[\s|:-]+$/.test(lines[i + 1])
...
function renderInline(text: string): React.ReactNode[] {
  // Handle bold, italic, inline code, and links using a token-based approach
  // Pattern precedence: bold (**) > inline code (`) > link ([]) > italic (*)
  ...
  while (remaining.length > 0) {
    ...
    // Italic: *text* (single asterisks -- must not be part of **)
    const italicMatch = remaining.match(/(?:^|[^*])\*([^*]+?)\*(?:[^*]|$)/);
    if (italicMatch && italicMatch.index != null) {
      // Adjust: the match may start one char before the *
    ...
    // Push prefix text before the match
    if (earliest.index > 0) {
    ...
    if (earliest.type === "bold") {
      ...
      // For italic, the matched content is in group 1 of the original regex
      // But we need the text between the single asterisks
      ...
      // Skip past the *text* (including both asterisks)
...
function ReportSkeleton() {
  return (
    <div className="bg-[rgba(10,15,26,0.6)] backdrop-blur-[12px] border border-border rounded-xl p-8 space-y-6 animate-fade-in">
      {/* Title */}
      ...
      <div className="space-y-4">
        ...
        <div className="h-6 w-1/3 bg-white/10 rounded animate-pulse mt-4" />
...
export default function InvestmentThesisReport({ screenId }: InvestmentThesisReportProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getReport(screenId);
      setData(result);
    } catch (err) {
  ...
  const parsed = useMemo(() => {
    if (!data?.report?.content) return { blocks: [], toc: [] };
    return parseMarkdown(data.report.content);
  }, [data]);
    ...
    return (
      <div className="bg-[rgba(10,15,26,0.6)] backdrop-blur-[12px] border border-border rounded-xl px-6 py-12 text-center">
        ...
        <p className="text-sm text-text-secondary max-w-sm mx-auto">
          The investment thesis report will be available here once the
          synthesis phase completes and the report is generated.
        </p>
      </div>
    );
  }
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-[rgba(10,15,26,0.6)] backdrop-blur-[12px] border border-border rounded-xl p-6 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-text-secondary mt-2">
              ...
              {meta.model && (
                <span className="text-text-tertiary"> via {meta.model}</span>
              )}
            </p>
          </div>
          <button
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          <MetadataBadge icon={Layers} label="Pillars" value={meta.pillarCount} />
          <MetadataBadge icon={BarChart3} label="Equities" value={meta.equityCount} />
          <MetadataBadge icon={Trophy} label="Tier 1" value={meta.tier1Count} />
          <MetadataBadge icon={Users} label="Tier 2" value={meta.tier2Count} />
          <MetadataBadge icon={Hash} label="Tier 3" value={meta.tier3Count} />
          <MetadataBadge icon={Clock} label="Words" value={meta.wordCount.toLocaleString()} />
        </div>
      </div>
